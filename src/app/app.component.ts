...
export class AppComponent {
  ...
  updateMessage(newMessage: string) {
    ...
    setTimeout(() => this.resetMessage(), 1);
  }
  ...
  restartInterval() {
    ...
    if(this.isRunning) {
      this.intervalID = setInterval(() => this.updateMessage('nextStep'), 50000 / this.speed);
    }
  }
   ...
   */
  exportSession(exportToken: string) {
  ...
  toClipboard() {
    this.isExport = false;
    let copyString = document.querySelector('#copystring') as HTMLInputElement;
